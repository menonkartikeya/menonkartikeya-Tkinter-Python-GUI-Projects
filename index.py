...
try:
        ...
        host="localhost",
        user="root",
        password="", #give your mysql password here
        database="Contacts",
        autocommit=True)
except:
        mydb=mysql.connector.connect(
        ...
        messagebox.showinfo("Created!", "Database Created successfully. Now exit, ignore the error and rerun the program")


mycursor=mydb.cursor()
mycursor.execute("CREATE TABLE IF NOT EXISTS contactsdata (id INT AUTO_INCREMENT PRIMARY KEY,\
...
mycursor=mydb.cursor()
mycursor.execute("CREATE TABLE IF NOT EXISTS grouplist (id INT AUTO_INCREMENT PRIMARY KEY,\
...
if not result:
        mycursor.executemany("INSERT INTO grouplist (group_name,extra_id) VALUES (%s,%s)",val)
        mydb.commit()
...
def mainapplication():
    ...
    def ContactEditor(id_reference):
        ...
        #When User is editing a contact... the previous data will be inserted in entry boxes prior like this:
            if(id_reference!=-1):
                mycursor=mydb.cursor()
                result_new = mycursor.execute("SELECT * FROM contactsdata WHERE id = %s",(id_reference,))
                result_new = mycursor.fetchall()
                EFirstName.insert(0,result_new[0][1])
                EMiddleName.insert(0,result_new[0][2])
                ELastName.insert(0,result_new[0][3])
                ENickName.insert(0,result_new[0][4])
                VGender.set(result_new[0][5])
                EDOB.insert(0,result_new[0][6])
                EAddress.insert(0,result_new[0][7])
                ECity.insert(0,result_new[0][8])
                EState.insert(0,result_new[0][9])
                EPIN.insert(0,result_new[0][10])
                ECountry.insert(0,result_new[0][11])
                EPhone1.insert(0,result_new[0][12])
                EPhone2.insert(0,result_new[0][13])
                EPhone3.insert(0,result_new[0][14])
                EEmail1.insert(0,result_new[0][15])
                EEmail2.insert(0,result_new[0][16])
                Ewebs.insert(0,result_new[0][17])

                G_index = result.index(result_new[0][18])  #To get the group_name's index in list and make it default
            ...
            if(id_reference==-1):  #Adding a new contact
                    ...
                    def saveit():
                        ...
                        sql = "SELECT * FROM contactsdata WHERE firstname = %s"      
                        name = (VFirstName.get(),)
                        mycursor=mydb.cursor()
                        ...
                        if(VFirstName.get()==""):
                            ...
                            sql1= "INSERT INTO contactsdata (firstname,middlename,lastname,nickname,gender,dob,address,city,state,pin,country,phone1,phone2,phone3,email1,email2,webs,contactgroup) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                    ...
                    def saveit():
                        if(VFirstName.get()==""):
                            ...
                            sql2= "UPDATE contactsdata SET firstname = %s,middlename = %s ,lastname=%s,nickname=%s,gender=%s,dob=%s,address=%s,city=%s,state=%s,pin=%s,country=%s,phone1=%s,phone2=%s,phone3=%s,email1=%s,email2=%s,webs=%s,contactgroup=%s WHERE id = %s "
    ...
    def delete_all():
            ...
            if option:
                    mycursor = mydb.cursor()
                    Delete_all_rows = "TRUNCATE TABLE contactsdata"
    ...
    def exportcsv():
        ...
        result=mycursor.fetchall()
        with open('Contacts_Data.csv','a') as f:
    ...
    def add_edit_group():
            ...
            def search_and_next():
                    ...
                    sql = "SELECT * FROM grouplist WHERE group_name = %s"
                    name = (searched,)
                    mycursor = mydb.cursor()
                    ...
                    if searched=="": #Nothing Entered
                        ...
                        def add_group():
                                mycursor = mydb.cursor()
                                sql = "INSERT INTO grouplist (group_name,extra_id) VALUES (%s,%s)"
                        ...
                        def edit_group():
                                if(VGroup_name.get()==""):
                                    ...
                                    mycursor = mydb.cursor()
                                    sql= "UPDATE grouplist SET group_name = %s,extra_id = %s WHERE group_name = %s "
                        ...
                        def del_group():
                                mycursor = mydb.cursor()
                                sql= "DELETE FROM grouplist WHERE group_name = %s "
                                val= searched
                                mycursor.execute(sql,(val,))
                                mydb.commit()
    ...
    mycursor9 = mydb.cursor(buffered=True) #Without buffered, it will give error as cursor is used again in canvas2
    ...
    def DeleteContact(id_reference):
        if(id_reference==-1):
            pass
                ...
                mycursor1 = mydb.cursor()
                sql3= "DELETE FROM contactsdata WHERE id = %s "
    ...
    def create_canvas2():
        mycursor = mydb.cursor()
        mycursor.execute("SELECT * FROM contactsdata ORDER BY firstname")
        result=mycursor.fetchall()
        for index,x in enumerate(result):
            col=0.1
